...
$tableCheckQuery = "SHOW TABLES LIKE 'Timetable'";
$tableCheckResult = $conn->query($tableCheckQuery);

if ($tableCheckResult && $tableCheckResult->num_rows > 0) {
    echo "Table 'timetable' already exists. Skipping creation.";
} else {
...
if ($conn->query($createTableSQL) === TRUE) {
    echo "Table 'Timetable' created successfully.\n";
} else {
...
$insertTimetableSQL = "INSERT IGNORE INTO Timetable (date, time, task, status) VALUES 
...
if ($conn->query($insertTimetableSQL) === TRUE) {
    echo "Data inserted successfully (if not duplicate)!\n";
} else {
